...
class AVLTree(object):
	...
	def rotate_right(self, b):  # Rotates nodes to the right as learned in lecture, O(1)
		if self.root.get_key() == b.get_key():  # if rotation is at root, then set the new root
			self.root = b.get_left()

		a = b.get_left()
		b.set_left(a.get_right())  # Code from lecture
	...
	def rotate_left(self, b):  # Rotates nodes to the left as learned in lecture, O(1)
		if self.root.get_key() == b.get_key():  # if rotation is at root, then set the new root
			self.root = b.get_right()

		a = b.get_right()
		b.set_right(a.get_left())  # Code from lecture
	...
	def insert(self, key, val):  # Insert new node into self, O(log n)
		...
		insert_me = AVLNode(key, val)  # Create new AVLNode with virtual children
		insert_me.set_left(AVLNode(None, None))
		insert_me.get_left().set_parent(insert_me)
		insert_me.set_right(AVLNode(None, None))
		insert_me.get_right().set_parent(insert_me)
		insert_me.set_height(0)
		insert_me.set_size(1)

		if self.root is None:  # Tree is empty
			self.root = insert_me  # insert_me is root of tree
			...
			self.tree_insert(self.root, insert_me)

		# Rotations:
		parent = insert_me.get_parent()
		while parent is not None:  # We don't set a virtual parent, so trying to access root's parent we get None
			...
			if abs(bf) < 2:
				if parent.get_height() == prev_height:  # Algorithm from lecture, line 3.2
					return cnt_balance
				else:  # Algorithm from lecture, line 3.3 (parent's height changed)
